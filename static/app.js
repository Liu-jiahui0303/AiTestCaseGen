...
function loadSessions(){try{const s=localStorage.getItem(STORE_KEY);if(s){const d=JSON.parse(s);if(d.length) sessions=d;}}catch(e){}}
function saveSessions(){localStorage.setItem(STORE_KEY,JSON.stringify(sessions));}
function getSession(){return sessions.find(s=>s.id===activeSessionId)||sessions[0];}
function switchSession(sid){
  activeSessionId=sid;const s=getSession();
